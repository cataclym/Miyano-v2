...
exports.run = async (client, message, args) => {
	...
	const res = await client.db(`select * from users where user_id = '${toView.user.id}'`);
	const background = res[0].background_id || 0;

	const rank = await client.db("select * from users ORDER by xp desc");
	let xp;
				...
				Jimp.loadFont(Jimp.FONT_SANS_16_WHITE).then(function(font) {
					Jimp.loadFont(Jimp.FONT_SANS_32_WHITE).then(function(fontBIG) {
						Jimp.loadFont(Jimp.FONT_SANS_8_WHITE).then(function(font2) {
							...
							// Creates the file
							const outputFile = "./images/" + Math.random().toString(36).substr(2, 5) + "test." + image.getExtension();
