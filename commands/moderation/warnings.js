...
module.exports.run = async (client, message, args) => {
	...
	if (!args[0]) {
		...
		if (res.length == 0) {
			...
			for (i = 0;i < numb;i++) {
				const target = await client.findUser(message, res[i].user_id);
				const mod = await client.findUser(message, res[i].mod_id);

				if (target && mod) {str += `**Case: ${res[i].id}** - User **__${target.username}#${target.discriminator}__** warned at ${moment(res[i].time).format("MMMM Do YYYY, h:mm:ss a")}\n**Reason:** ${res[i].reason}\n**Warned By:** ${mod.username}#${mod.discriminator}\n\n`;}
			}
		...
		const target = await client.findUser(message, text);
