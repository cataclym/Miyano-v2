exports.run = async (client, message, args) => {
	...
	const res = await query(`SELECT * FROM autoassign WHERE guild_id = '${message.guild.id}'`);

	if (!args[0]) {
		if (!res[0]) return message.channel.send("There is not an auto assigned role set!");
		const role = message.guild.roles.get(res[0].role);
	...
	if (args[0] == "clear") {
		...
		const role = message.guild.roles.find(val => val.name.toLowerCase() === text.toLowerCase());
