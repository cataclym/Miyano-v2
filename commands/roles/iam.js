...
module.exports.run = async (bot, message, args, db) => {
	...
	if (message.guild.id == "204487943568621568") {
		...
		if (myRole.id == "336786415407071233" || myRole.id == "206668607327961088" || myRole.id == "271875615064326146" || myRole.id == "322165288332165121" || myRole.id == "220998153044885505" || myRole.id == "338795067097743361" || myRole.id == "418555569670979584") {
			...
			for (i = 0;i < numbRoles;i++) {
				if (rows[i].role_id == "336786415407071233" || rows[i].role_id == "206668607327961088" || rows[i].role_id == "271875615064326146" || rows[i].role_id == "322165288332165121" || rows[i].role_id == "220998153044885505" || rows[i].role_id == "418555569670979584" || rows[i].role_id == "338795067097743361") {roleCount += 0;}
				else {
					const userRole = message.guild.roles.find("id", rows[i].role_id);
		...
		if (myRole.id == "403074596711956491" || myRole.id == "412080053598289922" || myRole.id == "431294268158640141" || myRole.id == "430150559421693962" || myRole.id == "438872437816295424") {
			...
			for (i = 0;i < numbRoles;i++) {
				if (rows[i].role_id == "403074596711956491" || rows[i].role_id == "412080053598289922" || rows[i].role_id == "431294268158640141" || rows[i].role_id == "430150559421693962" || rows[i].role_id == "438872437816295424") {roleCount += 0;}
				else {
					const userRole = message.guild.roles.find("id", rows[i].role_id);
					if (message.member.roles.has(userRole.id)) {
						roleCount += 1;
	...
	async function getRole(roleCount) {
		...
		const res = await query(`SELECT * FROM roles WHERE role_id='${myRole.id}' AND guild_id='${message.guild.id}';`);
		...
		if (!res[0]) {
			...
			const msg = await message.channel.send(embedSuccess(`${message.author.username}, you now have the ${myRole.name} role.`));
		}
	}
};
